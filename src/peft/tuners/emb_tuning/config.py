...
@dataclass
class EMBTonfig(PeftConfig):
    """
    This is the configuration class to store the configuration of a [`EMBTModel`].
    ...
    Args:
        ...
        feedforward_modules (`Optional[Union[List[str], str]]`):
            ...
            have EMBT vectors added to the input, instead of the output. `feedforward_modules` must be a name or
            a subset of names present in `target_modules`.
        fan_in_fan_out (`bool`):
            Set this to True if the layer to replace stores weight like (fan_in, fan_out). For example, gpt-2 uses
            `Conv1D` which stores weights like (fan_in, fan_out) and hence this should be set to `True`.
        modules_to_save (`Optional[List[str]]`):
            List of modules apart from EMBT layers to be set as trainable and saved in the final checkpoint.
        init_embt_weights (`bool`):
            Whether to initialize the vectors in the EMBT layers, defaults to `True`. Setting this to `False` is
            discouraged.
    """
    ...
    target_modules: Optional[Union[List[str], str]] = field(
        ...
        metadata={
            "help": (
                "List of module names or regex expression of the module names to replace with EMBT."
    ...
    modules_to_save: Optional[List[str]] = field(
        default=None,
        metadata={
            "help": "List of modules apart from EMBT layers to be set as trainable and saved in the final checkpoint. "
            "For example, in Sequence Classification or Token Classification tasks, "
            "the final layer `classifier/score` are randomly initialized and as such need to be trainable and saved."
        },
    )
    init_embt_weights: bool = field(
        default=True,
        metadata={"help": "Whether to initialize the vectors in the EMBT layers."},
    )

    def __post_init__(self):
        self.peft_type = PeftType.EMBT
        self.target_modules = (
